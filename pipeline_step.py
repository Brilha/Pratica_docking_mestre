...
class PipelineStep(ABC):
    ...
    def validate_inputs(self) -> bool:
        """Verifica se arquivos de entrada existem (com suporte a glob patterns)"""
        required = self.get_required_files()
        
        if not required:
            return True
        
        # ✅ NOVO: Expandir padrões glob como *_clean.pdb
        full_paths = []
        for f in required:
            full_path = str(self.work_dir / f)
            # Detectar se é padrão glob (contém * ou ?)
            if '*' in f or '?' in f:
                matches = glob.glob(full_path)
                if matches:
                    full_paths.extend(matches)
                else:
                    full_paths.append(full_path)  # Será detectado como missing abaixo
            else:
...
class PythonScriptStep(PipelineStep):
    ...
    def execute_impl(self) -> ProcessResult:
        ...
        # ✅ REMOVIDO: cor_pdb_seletividade.py agora é modular (detecta *_clean.pdb automaticamente)
